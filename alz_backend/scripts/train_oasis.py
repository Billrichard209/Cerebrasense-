"""Run the research-grade OASIS-1 MONAI training pipeline."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Train the OASIS-1 binary 3D MRI baseline with MONAI.")
    parser.add_argument("--config", type=Path, default=None, help="Path to an OASIS training YAML config.")
...
def main() -> None:
    ...
    cfg = load_research_oasis_training_config(args.config)
    cfg = apply_cli_overrides(cfg, args)
    result = run_research_oasis_training(cfg)
